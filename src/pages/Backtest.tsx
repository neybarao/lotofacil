...
export function Backtest({ draws }: { draws: Draw[] }) {
  ...
  const result = useMemo(() => runBacktest(draws, targetContest), [draws, targetContest])
  ...
  return (
    <div className="page-stack">
      <section className="page-intro page-intro--split">
        <div><p className="eyebrow">Teste retroativo</p><h1>Analisar sem conhecer o resultado</h1><p className="intro-copy">A base é cortada antes do concurso escolhido; só depois comparamos os jogos.</p></div>
        <div className="field"><label htmlFor="target-contest">Concurso testado</label><select id="target-contest" value={targetContest} onChange={(event) => setTargetContest(Number(event.target.value))}>{recentTargets.map((draw) => <option key={draw.n} value={draw.n}>{draw.n} · {draw.d}</option>)}</select></div>
      </section>
      ...
      <section className="backtest-result">
        ...
        <div>
          ...
          <p>Melhor resultado</p><strong>{best} acertos em 15</strong>
        </div>
      </section>
