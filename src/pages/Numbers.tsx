  ...
  return (
    <div className="page-stack">
      <section className="page-intro page-intro--split">
        <div><p className="eyebrow">Dezenas</p><h1>Frequência e atraso</h1><p className="intro-copy">Atraso é a quantidade de concursos desde a última aparição.</p></div>
      ...
      <section className="number-table" aria-label="Estatísticas das dezenas">
        <div className="number-table__header"><span>Dezena</span><span>Frequência</span><span>Atraso</span><span>Máximo</span></div>
        {stats.map((stat, index) => (
          <div className="number-table__row" key={stat.number} style={{ '--index': index } as React.CSSProperties}>
            ...
            <span><strong>{stat.maxDelay}</strong><small>concursos</small></span>
          </div>
        ))}
      </section>
    </div>
