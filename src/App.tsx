    ...
    <div className="app-shell">
      <header className="topbar">
        <button className="brand" type="button" onClick={() => setPage('dashboard')} aria-label="Ir para o painel">
          ...
          <span><strong>Lotofácil</strong><small>análise estatística</small></span>
        </button>
      ...
      {showInfo && <div className="modal-backdrop" role="presentation" onMouseDown={() => setShowInfo(false)}><section className="info-sheet" role="dialog" aria-modal="true" aria-labelledby="info-title" onMouseDown={(event) => event.stopPropagation()}><button className="icon-button info-sheet__close" type="button" onClick={() => setShowInfo(false)} aria-label="Fechar"><X size={22} /></button><ClockCounterClockwise size={28} /><p className="eyebrow">Base de dados</p><h2 id="info-title">Atualização e origem</h2><dl><div><dt>Último concurso</dt><dd>{data.metadata.latestContest} · {data.metadata.latestDrawDate}</dd></div><div><dt>Concursos carregados</dt><dd>{data.metadata.contestCount.toLocaleString('pt-BR')}</dd></div><div><dt>XLSX atualizado</dt><dd>{updatedAt}</dd></div></dl><a href={data.metadata.source} target="_blank" rel="noreferrer">Abrir página oficial da CAIXA</a><p className="fine-print">A aplicação funciona sem API. Os cálculos usam somente o arquivo incluído nesta versão.</p></section></div>}
    </div>
  )
}

function LoadingScreen() {
  return <main className="state-screen"><div className="state-logo"><img className="brand-mark" src={`${import.meta.env.BASE_URL}icons/icon-192-v2.png`} alt="" /></div><div className="skeleton skeleton--title" /><div className="skeleton skeleton--copy" /><div className="skeleton skeleton--panel" /><p>Preparando as estatísticas…</p></main>
}

function ErrorScreen({ message }: { message: string }) {
  return <main className="state-screen"><span className="state-error">!</span><h1>Não foi possível abrir a base</h1><p>{message}</p><button className="button button--primary" type="button" onClick={() => window.location.reload()}>Tentar novamente</button></main>
}
